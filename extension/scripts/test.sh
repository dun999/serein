...
#   VAULT_ADDRESS       — initialized CovenantVault assigned to this extension
#   TEST_RECIPIENT      — EVM address allowed by the vault's encrypted policy
#   TEST_AMOUNT         — FXRP base units to authorize (default: 1000000)
set -euo pipefail
...
TEST_AMOUNT="${TEST_AMOUNT:-1000000}"

[[ -n "$INSTRUCTION_SENDER" ]] || die "INSTRUCTION_SENDER not set. Run pre-build.sh first or set it manually."
[[ "$VAULT_ADDRESS" =~ ^0x[0-9a-fA-F]{40}$ ]] || die "VAULT_ADDRESS must be an initialized CovenantVault address."
[[ "$TEST_RECIPIENT" =~ ^0x[0-9a-fA-F]{40}$ ]] || die "TEST_RECIPIENT must be allowed by the private policy."
[[ "$TEST_AMOUNT" =~ ^[1-9][0-9]*$ ]] || die "TEST_AMOUNT must be positive FXRP base units."
...
log "Test recipient:     $TEST_RECIPIENT"
