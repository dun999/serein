...
# Dependency pins must agree before anything is built; drift surfaces later as
# an opaque signature-verification failure.
...
# register-extension also performs and verifies the sender's one-time
# setExtensionId() initialization. A registered but uninitialized sender would
# make every vault request revert with ExtensionIdNotSet.
