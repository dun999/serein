...
#
# Coston2 normally requires EXT_PROXY_URL to be a stable public HTTPS endpoint.
# For short-lived testnet trials only, ALLOW_EPHEMERAL_TUNNEL=true permits a
# Cloudflare quick tunnel. Restarting it requires re-registering the new URL.
#
# Usage:
#   ./scripts/start-services.sh                       # local devnet, docker compose
#   ./scripts/start-services.sh --local               # local devnet, Go processes
#   ./scripts/start-services.sh --chain coston2               # use configured stable endpoint
#   ./scripts/start-services.sh --chain coston2 --tunnel      # start configured named tunnel
#
# The node and proxy use pinned, self-contained dependency versions.
#
# Prerequisites:
...
# The well-known Anvil identity is convenient on the isolated local devnet, but
# must never silently become the proxy/governance identity on a public chain.
if [[ "$CHAIN" == "local" ]]; then
    PROXY_PRIVATE_KEY="${PROXY_PRIVATE_KEY:-983760a4ebf75b2ac3a93531168a0f225d01e5dc6e3568adbd46233ba1fb4fa4}"
    INITIAL_OWNER="${INITIAL_OWNER:-0xF4E021377420Afe90c1A7D2b8968904946633a64}"
else
    [[ -n "$PROXY_PRIVATE_KEY" ]] || die "PROXY_PRIVATE_KEY must be set for $CHAIN; no public-network default is permitted."
    [[ -n "$INITIAL_OWNER" ]] || die "INITIAL_OWNER must be set for $CHAIN; no public-network default is permitted."
fi
export PROXY_PRIVATE_KEY INITIAL_OWNER
...
# Covenant has one authoritative workload implementation.
[[ "$LANGUAGE" == "go" ]] || die "LANGUAGE must be go (got '$LANGUAGE')"
...
if [[ "$CHAIN" == "local" ]]; then
    ...
    # Providers deliver directly to the URL stored on-chain. A rotating quick
    # tunnel strands the machine after the hostname changes, so public FCC
    # deployments require an explicitly configured stable HTTPS endpoint unless
    # the operator deliberately opts into an ephemeral hackathon test session.
    ALLOW_EPHEMERAL_TUNNEL="${ALLOW_EPHEMERAL_TUNNEL:-false}"
    if [[ "$ALLOW_EPHEMERAL_TUNNEL" != "true" ]]; then
        [[ -n "${EXT_PROXY_URL:-}" ]] || die "EXT_PROXY_URL must be a stable public HTTPS URL for Coston2."
        [[ "$EXT_PROXY_URL" != *".trycloudflare.com"* ]] || die "Cloudflare quick-tunnel URLs rotate and must not be registered on Coston2. Use a named tunnel or set ALLOW_EPHEMERAL_TUNNEL=true for a short-lived test."
    elif [[ "$USE_TUNNEL" == "true" && -z "${TUNNEL_ARGS:-}" ]]; then
        log "WARNING: ephemeral Cloudflare tunnel enabled for this Coston2 test session."
    fi
    if [[ "$USE_TUNNEL" == "true" && -z "${TUNNEL_ARGS:-}" && "$ALLOW_EPHEMERAL_TUNNEL" != "true" ]]; then
        die "--tunnel without TUNNEL_ARGS creates a rotating quick tunnel. Configure a named tunnel and stable EXT_PROXY_URL instead."
    fi
    sync_tunnel
    [[ -n "${EXT_PROXY_URL:-}" ]] || die "EXT_PROXY_URL was not discovered or configured for Coston2."
    [[ "$EXT_PROXY_URL" == https://* ]] || die "EXT_PROXY_URL must use HTTPS for Coston2 (got: $EXT_PROXY_URL)."
fi
...
if [[ "$USE_LOCAL" == "false" ]]; then
    ...
    if [[ -z "${REGISTRY:-}" ]]; then
        if ! docker image inspect local/tee-proxy >/dev/null 2>&1; then
            ...
            log "Building local/tee-proxy from $PROXY_DOCKERFILE (pinned, self-contained)..."
