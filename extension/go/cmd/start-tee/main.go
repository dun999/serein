...
// the combined Covenant workload and points at Docker Compose instead.
package main
...
func runExtension() {
	// The local combined process uses the same registered TEE signing path as
	// Docker/FCC, rather than an application-owned development key.
	os.Setenv("TEE_SIGN_URL", fmt.Sprintf("http://127.0.0.1:%d/sign", ExtensionServerPort))
	...
	if enclaveErr != nil {
		logger.Fatalf("covenant enclave: %v", enclaveErr)
	}

	// Start extension server — fail fast if port binding fails.
	extErrCh := extserver.StartExtension(ExtensionPort, ExtensionServerPort, covenantEnclave)
