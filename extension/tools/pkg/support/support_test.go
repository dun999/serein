...
func TestRequireDeploymentPrivateKeyRejectsMissingKeyOnPublicChain(t *testing.T) {
	t.Setenv("DEPLOYMENT_PRIVATE_KEY", "")

	err := requireDeploymentPrivateKey(big.NewInt(114))
	if err == nil || !strings.Contains(err.Error(), "must be set for chain ID 114") {
		t.Fatalf("expected public-chain key error, got %v", err)
	}
}

func TestRequireDeploymentPrivateKeyAllowsConfiguredKey(t *testing.T) {
	t.Setenv("DEPLOYMENT_PRIVATE_KEY", "01")

	if err := requireDeploymentPrivateKey(big.NewInt(114)); err != nil {
		t.Fatalf("expected configured key to be accepted, got %v", err)
	}
}

func TestRequireDeploymentPrivateKeyAllowsLocalDevChain(t *testing.T) {
	t.Setenv("DEPLOYMENT_PRIVATE_KEY", "")

	if err := requireDeploymentPrivateKey(big.NewInt(31337)); err != nil {
		t.Fatalf("expected local development key to be accepted, got %v", err)
	}
}
