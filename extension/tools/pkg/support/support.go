...
func DefaultSupport(AddressesFilePath, chainNodeURL string) (*Support, error) {
	...
	if err != nil {
		cc.Close()
		return nil, errors.Errorf("cannot resolve chain ID: %s", err)
	}
	if err := requireDeploymentPrivateKey(chainID); err != nil {
		cc.Close()
		return nil, err
	}
...
func requireDeploymentPrivateKey(chainID *big.Int) error {
	if strings.TrimSpace(os.Getenv("DEPLOYMENT_PRIVATE_KEY")) != "" {
		return nil
	}

	// The bundled development key is intentionally restricted to the isolated
	// Anvil chain. A public testnet account can be funded accidentally, which
	// must never turn a missing secret into an authorized live deployment.
	if chainID.Cmp(big.NewInt(31337)) == 0 {
		return nil
	}

	return errors.Errorf(
		"DEPLOYMENT_PRIVATE_KEY must be set for chain ID %s; the bundled development key is allowed only on local chain 31337",
		chainID.String(),
	)
}
