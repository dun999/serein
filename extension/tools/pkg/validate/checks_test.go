...
func TestRegisterTeeMachineChecks_SimulatedTeeHackathonProfile(t *testing.T) {
	...
	foundPass := false
	for _, res := range r.Results {
		if res.ID == "T1" && res.Status == PASS {
			foundPass = true
		}
	}
	if !foundPass {
		t.Fatal("expected T1 PASS for the supported Coston2 hackathon profile")
	}
}
