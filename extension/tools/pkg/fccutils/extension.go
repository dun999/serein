...
// InitializeInstructionSender binds the sender to the public extension that
// was just registered for it. Without this one-time transaction every request
// reverts with ExtensionIdNotSet even though the registry entry is valid.
func InitializeInstructionSender(
	s *support.Support, instructionsSenderAddress common.Address, expectedExtensionID *big.Int,
) error {
	sender, err := covenant.NewCovenantInstructionSender(instructionsSenderAddress, s.ChainClient)
	if err != nil {
		return errors.Errorf("bind InstructionSender: %s", err)
	}

	opts, err := bind.NewKeyedTransactorWithChainID(s.Prv, s.ChainID)
	if err != nil {
		return errors.Errorf("create InstructionSender transactor: %s", err)
	}
	tx, err := sender.SetExtensionId(opts)
	if err != nil {
		return errors.Errorf("InstructionSender.SetExtensionId failed: %s", err)
	}
	if _, err = support.CheckTx(tx, s.ChainClient); err != nil {
		return errors.Errorf("InstructionSender.SetExtensionId transaction failed: %s", err)
	}

	actual, err := sender.ExtensionId(&bind.CallOpts{Context: context.Background()})
	if err != nil {
		return errors.Errorf("read initialized InstructionSender extension ID: %s", err)
	}
	if actual.Cmp(expectedExtensionID) != 0 {
		return errors.Errorf("InstructionSender extension ID mismatch: got %s, want %s", actual, expectedExtensionID)
	}
	logger.Infof("InstructionSender initialized with extension ID %s", actual)
	return nil
}
